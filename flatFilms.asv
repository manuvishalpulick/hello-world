...
%% equation
%                                                           ___
%  d H      d  /   3 d^3 H     1 dH    4  1  dH   \       __    d  /    /           \
%  --- = - ---|   H  ----- +   ----- - -.---.--    |  + _/2T   ---|    / H^3 N(X,T)  |
%  d T     d X \     d X^3     H dX    3 H^2 dX   /           d X \ \/             / 
...
%    h3_r =  0.5.*h_min.*(1./(h(k(3:h_adjusted-2)).^2) + 1./(h(k(4:h_adjusted-1)).^2).*(h(k(4:h_adjusted-1)) - h(k(3:h_adjusted-2))); % see the discretization

%    h3_l =  0.5.*h_min.*(1./(h(k(3:h_adjusted-2)).^2) + 1./(h(k(2:h_adjusted-3)).^2)).*(h(k(3:h_adjusted-2)) - h(k(2:h_adjusted-3))); % see the discretization
